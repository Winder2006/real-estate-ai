...
try:
    ...
    # Try creating a simple test class instead
    print("🔧 Creating fallback Excel generator...")
    try:
        class SimpleExcelGenerator:
            def create_pro_forma(self, property_data, results, assumptions, project_name):
                ...
                worksheet.write('A1', 'Test Excel Generation')
                worksheet.write('A2', f'Project: {project_name}')
                worksheet.write('A3', f'Property: {property_data.get("address", "Unknown")}')
                
                workbook.close()
                output.seek(0)
                return output.getvalue()
        
        RealEstateExcelGenerator = SimpleExcelGenerator
        print("✅ Created fallback Excel generator")
